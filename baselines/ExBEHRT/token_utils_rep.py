...
class EHRTokenizer(object):
    def __init__(self, diag_sentences, lab_sentences, pro_sentences, 
        ...
        assert len(special_tokens) + len(self.age_voc.id2word) + \
            len(self.diag_voc.id2word) + len(self.lab_voc.id2word) + len(self.pro_voc.id2word) + \
    ...
    def token_id_range(self, voc_type="diag"):
        ...
        pro_size = len(self.pro_voc.id2word)

        if voc_type == "diag":
            return [self.n_special_tokens + age_size, self.n_special_tokens + age_size + diag_size]
        elif voc_type == "lab":
            return [self.n_special_tokens + age_size + diag_size, self.n_special_tokens + age_size + diag_size + lab_size]
        elif voc_type == "pro":
            return [self.n_special_tokens + age_size + diag_size + lab_size, len(self.vocab.id2word)]

...
class PoswiseFeedForwardNet(nn.Module):
    def __init__(self, config):
        ...
        self.fc1 = nn.Linear(config["gt_hidden_size"], config["gt_hidden_size"])
        self.fc2 = nn.Linear(config["gt_hidden_size"], config["gt_hidden_size"])
        self.dropout = nn.Dropout(config["gt_hidden_dropout_prob"])
...
class MultiHeadAttention(nn.Module):
    def __init__(self, config):
        ...
        self.d_k, self.n_heads = config["gt_hidden_size"] // config["gt_num_attention_heads"], config["gt_num_attention_heads"]

        self.W_Q = nn.Linear(config["gt_hidden_size"], config["gt_hidden_size"])
        self.W_K = nn.Linear(config["gt_hidden_size"], config["gt_hidden_size"])
        self.W_V = nn.Linear(config["gt_hidden_size"], config["gt_hidden_size"])
        self.W_output = nn.Linear(config["gt_hidden_size"], config["gt_hidden_size"])
        self.dropout = nn.Dropout(config["gt_attention_dropout_prob"])

    def ScaledDotProductAttention(self, Q, K, V, attn_mask, inductive_bias: None):
        scores = torch.matmul(Q, K.transpose(-1, -2)) / math.sqrt(Q.size(-1))  # [B, H, L, L]
        if inductive_bias is not None:
            scores = scores + inductive_bias.unsqueeze(1)
        scores = scores.masked_fill(attn_mask, -1e4)

        attn = self.dropout(F.softmax(scores, dim=-1))  # [B, H, L, L]
        context = torch.matmul(attn, V)

        # scores is of shape [B, H, L, L]
        return context, attn, scores  # return scores for entropy regularization

    def forward(self, Q, K, V, attn_mask, inductive_bias):
        batch_size = Q.size(0)
        Q_proj = self.W_Q(Q)
        assert Q_proj.size(-1) == self.d_k * self.n_heads, "Mismatch in hidden dim vs n_heads * d_k"
        expected_elements = batch_size * Q.size(1) * self.n_heads * self.d_k
        assert Q_proj.numel() == expected_elements, "Incompatible shape for view()"
        ...
        context, _, scores = self.ScaledDotProductAttention(q_s, k_s, v_s, attn_mask, inductive_bias)
        ...
        return self.W_output(context), scores # return scores for entropy regularization, note that scores is multi-head
...
class GraphTransformerBlock(nn.Module):
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config["gt_hidden_dropout_prob"])
        self.norm_attn = nn.LayerNorm(config["gt_hidden_size"])
        self.norm_ffn = nn.LayerNorm(config["gt_hidden_size"])

    def forward(self, x, self_attn_mask, inductive_bias: None):
        norm_x = self.norm_attn(x)
        attn_logits, scores = self.self_attn(norm_x, norm_x, norm_x, self_attn_mask, inductive_bias)
        x = x + self.dropout(attn_logits)

        norm_x = self.norm_ffn(x)
        x = x + self.dropout(self.pos_ffn(norm_x))
        return x, scores  # return scores for entropy regularization
...
#         self.bn.reset_parameters()
    ...
    def forward(self, x, edge_index, norm, aggr='add', edge_weight=None):
        ...
        if self.attention:
            ...
            # print("HalfNLHconv: x shape: ", x.shape)
...
class Embedding(nn.Module):
    def __init__(self, vocab_size, hidden_size, pad_id, dropout_prob):

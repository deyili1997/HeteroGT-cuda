...
class HBERTFinetuneEHRDataset(Dataset):
    def __init__(self, data_pd, token_type, task):
        self.tokenizer = AutoTokenizer.from_pretrained("Charangan/MedBERT")
        self.task = task
    ...
    def __getitem__(self, item):
        ...
        input_text = [" ".join(input_token) for input_token in input_tokens]
        input_ids = [self.tokenizer(text, return_tensors="pt", truncation=True, padding=True, max_length=512)['input_ids'] for text in input_text]
        token_types = [torch.tensor([x]) for x in token_types]
        input_tokens = _pad_sequence(input_ids, pad_id=0)
        token_types = _pad_sequence(token_types, pad_id=0)
        n_adms = len(input_tokens)
        if n_adms > 1:
            # create a fully connected graph between admission
...
def batcher(pad_id, n_token_type=4, is_train=True):
    def batcher_dev(batch):
        ...
        raw_input_ids, raw_labels = [feat[0] for feat in batch], [feat[1] for feat in batch]
        ...
        if is_train:
            ...
            if raw_anomaly_labels[0] is not None:
                ...
                anomaly_labels = None
            return input_ids, labels, anomaly_labels
        else:
            ...
            return input_ids, labeled_batch_idx, labels

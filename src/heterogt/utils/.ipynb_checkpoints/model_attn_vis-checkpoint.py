...
class SampleVis():
    ...
    def visualize(self, input_ids, token_types, adm_index, age_ids, diag_code_group_dicts, labels, sample_idx):
        # only plot patients with more than 3 visits
        num_visits = (age_ids != self.age_pad_id).sum(dim=1)[sample_idx]
        if num_visits < 3:
            return
        ...
        SampleVis.visualize_tf_attn(tf_attn_probs[sample_idx],
                                   ...
                                   attn_score_fontsize = self.seq_plot_set["attn_score_fontsize"],
                                   label_fontsize = self.seq_plot_set["label_fontsize"],
                                   label_offset = self.seq_plot_set["label_offset"])
    ...
    def visualize_gnn(
        ...
        # 绘制边（使用基于注意力分数的宽度）
        nx.draw_networkx_edges(G, pos, arrows=True, width=edge_weights_visual, alpha=1.0,

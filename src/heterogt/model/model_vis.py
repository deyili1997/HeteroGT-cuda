...
from heterogt.model.layer_vis import DiseaseOccHetGNN, BinaryPredictionHead, MultiPredictionHead, HierTransformerLayer, CLSQueryMHA
import torch.nn.functional as F

class HeteroGTPreTrainVis(nn.Module):
    def __init__(self, tokenizer, token_types, d_model, num_heads, layer_types,
        ...
        super(HeteroGTPreTrainVis, self).__init__()
        self.device = device
    ...
    def forward(self, input_ids, token_types, adm_index, age_ids, diag_code_group_dicts, labels):
        ...
        pipeline_out = self.run_pipeline(input_ids, token_types, adm_index, age_ids, diag_code_group_dicts, return_last_visit=True)
        out, last_visit_embed = pipeline_out["cls_embed"], pipeline_out["last_visit_embed"]
    ...
    def run_pipeline(self, input_ids, token_types, adm_index, age_ids, diag_code_group_dicts, 
        ...
        # collect attn prob matrixs
        tf_attn_probs = [] 
        gnn_attn_probs = []
        # for draw graph
        graph_structure_batches = []
        ...
        for i, layer_type in enumerate(self.layer_types):
            if layer_type == 'gnn': # the only purpose is just to update visit_embed
                ...
                gnn_out_dict, gnn_attn = self.stack_layers[i](hg_batch)   # 拿到 tuple
                gnn_out = gnn_out_dict['visit']                           # 再从 dict 里取 visit 节点的表示
                visit_embed = self.process_gnn_out(gnn_out, num_visits, V) # [B, V, d]
                gnn_attn_probs.append(gnn_attn)
                graph_structure_batches.append(hg_batch)
                ...
                h, tf_attn_prob = self.stack_layers[i](x, src_key_padding_mask, attn_mask_l) # [B, L+V, d]
                seq_embed, visit_embed = self.process_tf_out(h, L, V) # [B, L, d], [B, V, d]
                tf_layers_count += 1
                tf_attn_probs.append(tf_attn_prob)
            else:
        ...
        if self.use_cls_cat:
            cls = h[:, 0, :]
            agg, agg_attn_prob = self.cls_MHA(h, token_types_full)
        ...
        if return_last_visit and not return_group_dec_loss: # for pretrain
            ...
            return {"cls_embed": out, 
                    "last_visit_embed": last_visit_embed, 
                    "graph_batch": graph_structure_batches[-1], 
                    "gnn_attn": gnn_attn_probs[-1], 
                    "tf_attn": tf_attn_probs[-1], 
                    "agg_attn": agg_attn_prob,
                    "token_types_full": token_types_full,
                    "adm_index_full": adm_index_full,
                   }
...
class HeteroGTFineTuneVis(HeteroGTPreTrainVis):
    def __init__(self, tokenizer, token_types, d_model, num_heads, layer_types,
    ...
    def forward(self, input_ids, token_types, adm_index, age_ids, diag_code_group_dicts):
        pipeline_out = self.run_pipeline(input_ids, token_types, adm_index, age_ids, diag_code_group_dicts, return_group_dec_loss=True)
        out = pipeline_out["cls_embed"]
        logits = self.cls_head(out)
        return logits, pipeline_out
...
class HeteroGTPretrain(nn.Module):
    def __init__(self, tokenizer, token_types, d_model, num_heads, layer_types, max_num_adms, device, label_vocab_size, 
        ...
        for token_type in self.token_types:
            self.add_module(f"{token_type}_cls", MultiPredictionHead(out_dim, self.label_vocab_size[token_type]))
...
class HeteroGT(nn.Module):
    def __init__(self, tokenizer, token_types, d_model, num_heads, layer_types, max_num_adms, device, task, label_vocab_size, 
                 attn_mask_dicts, use_cls_cat):
        ...
        self.global_vocab_size = len(self.tokenizer.vocab.word2id)
        self.token_types = token_types
        self.n_type = len(token_types)  # diag, med, lab, pro
        self.d_model = d_model
        self.num_attn_heads = num_heads
        self.layer_types = layer_types
        self.seq_pad_id = tokenizer.convert_tokens_to_ids(["[PAD]"], voc_type="all")[0]  # 0

...
class FineTuneEHRDataset(Dataset):
    ...
    def sanity_check(self, hadm_id, input_ids, token_types, adm_index, age_ids, diag_group_codes):
        ...
        group_code_vals = token_types[0][group_code_idx]
        assert torch.all(group_code_vals == self.group_code_type_id), \
        f"These group code pos are not 5: { [p for p, v in zip(all_group_code_pos, group_code_vals.tolist()) if v != 5] }"

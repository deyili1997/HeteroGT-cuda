...
class HeteroGT(nn.Module):
    def __init__(self, tokenizer, d_model, num_heads, layer_types, max_num_adms, device, task, label_vocab_size):
        ...
        self.type_emb = nn.Embedding(self.n_type + 2, d_model, padding_idx=self.type_pad_id) # n_type already have [PAD], + 2 for group and visit
        self.adm_index_emb = nn.Embedding(self.max_num_adms + 2, d_model, padding_idx=self.adm_pad_id) # +2 for pad and group. Group is using max_num_adms + 1
